...
test('public protocol links match backend routes', async () => {
  ...
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.required.includes('ceo_review_questions'), true);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.required.includes('open_blockers'), true);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.required.includes('launch_window_label'), true);
  ...
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.properties.ceo_review_questions.minItems, 3);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.properties.open_blockers.maxItems, 4);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.properties.launch_window_label.maxLength, 80);
  ...
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.required.includes('decision_state'), true);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.properties.decision_state.enum.includes('review'), true);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.required.includes('decision_summary'), true);
  assert.equal(tokenLaunchCandidatesSchema.properties.candidates.items.properties.decision_summary.maxLength, 260);
...
test('public home keeps a short decision-screen rhythm', async () => {
  ...
  assert.doesNotMatch(appSource, /class="public-notification-feed home-feed-preview"/);
  assert.doesNotMatch(appSource, /class="home-value-strip"/);
  assert.doesNotMatch(appSource, /class="home-explain-strip"/);
  assert.doesNotMatch(appSource, /class="home-command-panel"/);
  assert.doesNotMatch(appSource, /homeLiveStats\.slice\(0, 2\)/);
...
test('public token pages expose airdrop, presale, and whitepaper routes', async () => {
  ...
  assert.match(appSource, /class="token-proof-map"/);
  assert.match(appSource, /v-for="row in publicTokenProofMapRows"/);
  assert.match(appSource, /const publicTokenProofMapRows = computed/);
  assert.match(appSource, /title: 'Research useful work'/);
  assert.match(appSource, /title: 'Research utility first'/);
  ...
  assert.match(appSource, /label: 'Send CEO brief', mobileLabel: 'Send brief', primary: true, icon: ArrowRight, command: 'token-launch-brief'/);
  assert.match(appSource, /label: 'Open claim workflow', mobileLabel: 'Claim', icon: Trophy, command: 'airdrop-claim'/);
  assert.match(appSource, /label: 'Send CEO brief', mobileLabel: 'Send brief', primary: true, icon: UserCheck, command: 'token-launch-brief'/);
  assert.match(appSource, /label: 'Open reserve workflow', mobileLabel: 'Reserve', icon: CircleDollarSign, command: 'presale-reserve'/);
  ...
  assert.match(appSource, /class="token-ceo-mobile-shortlist"/);
  assert.match(appSource, /v-for="\(\s*row,\s*index\s*\) in tokenCeoMobileShortlistRows"/);
  assert.match(appSource, /const tokenCeoMobileShortlistRows = computed\(\(\) => tokenCeoCandidateRows\.value\.slice\(0, 3\)\)/);
  ...
  assert.match(appSource, /const tokenCeoCandidateLaunchType = computed/);
  assert.match(appSource, /tokenLaunchCandidatesData\.value\?\.launch_type_filter/);
  assert.match(appSource, /if \(filter === 'airdrop' \|\| filter === 'presale'\) return filter/);
  ...
  assert.match(appSource, /const launchType = tokenCeoCandidateLaunchType\.value/);
  ...
  assert.match(appSource, /candidate\.ceo_review_questions/);
  assert.match(appSource, /candidate\.open_blockers/);
  assert.match(appSource, /candidate\.launch_window_label/);
  assert.match(appSource, /function tokenCeoCandidateQuestionsFallback/);
  assert.match(appSource, /function tokenCeoCandidateOpenBlockersFallback/);
  assert.match(appSource, /function tokenCeoLaunchWindowFallback/);
  assert.match(appSource, /class="token-ceo-candidate-review-packet"/);
  ...
  assert.match(appSource, /function tokenCeoResearchSourceLabel\(source = ''\)/);
  assert.match(appSource, /tokenLaunchCandidatesData\.value\?\.stats/);
  assert.match(appSource, /ledger proof row\(s\) before claims/);
  assert.match(appSource, /ledger proof row\(s\) before receipts/);
  ...
  assert.match(appSource, /\$\{candidateCount\} airdrop candidate\(s\), \$\{readyCount\} ready/);
  ...
  assert.match(cssSource, /Token CEO candidate final cut: one compact decision row for real airdrop\/presale candidates/);
  assert.match(cssSource, /\/\* Token CEO candidate final cut:[\s\S]*\.token-page-airdrop \.token-ceo-candidate-lane article,[\s\S]*\.token-page-presale \.token-ceo-candidate-lane article\s*\{[\s\S]*grid-template-columns: 32px minmax\(0, 1fr\) minmax\(150px, 178px\) !important;/);
  assert.match(cssSource, /\/\* Token CEO candidate final cut:[\s\S]*\.token-page-airdrop \.token-ceo-candidate-decisions,[\s\S]*\.token-page-presale \.token-ceo-candidate-decisions\s*\{[\s\S]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\) !important;/);
  assert.match(cssSource, /\/\* Token CEO candidate final cut:[\s\S]*@media \(max-width: 760px\)[\s\S]*\.token-page-airdrop \.token-ceo-candidate-next-action,[\s\S]*\.token-page-presale \.token-ceo-candidate-proof\s*\{[\s\S]*display: none !important;/);
  assert.match(cssSource, /\/\* Token CEO candidate final cut:[\s\S]*@media \(max-width: 430px\)[\s\S]*\.token-page-airdrop \.token-ceo-candidate-decisions,[\s\S]*\.token-page-presale \.token-ceo-candidate-decisions\s*\{[\s\S]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\) !important;/);
...
test('signed-in mobile dashboard keeps nav, actions, and popovers phone-safe', async () => {
  ...
  assert.match(cssSource, /Home final owner: compact premium first screen and hover menus that release cleanly/);
  ...
  assert.match(appSource, /class="action-label-full"/);
  assert.match(appSource, /class="action-label-mobile"/);
  assert.match(appSource, /mobileLabel: 'Send brief'/);
  assert.match(appSource, /mobileLabel: 'Bounties'/);
  assert.match(appSource, /mobileLabel: 'Reserve'/);
  assert.match(appSource, /mobileLabel: 'Ledger'/);
  ...
  assert.match(cssSource, /Token mobile action rail: keep secondary token actions compact so CEO candidates appear sooner/);
  assert.match(cssSource, /\/\* Token mobile action rail:[\s\S]*\.token-page-airdrop \.token-hero \.marketplace-actions,[\s\S]*\.token-page-presale \.token-hero \.marketplace-actions\s*\{[\s\S]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\) !important;/);
  assert.match(cssSource, /\/\* Token mobile action rail:[\s\S]*\.token-page-airdrop \.token-hero \.marketplace-actions \.large:first-child,[\s\S]*grid-column: 1 \/ -1 !important;/);
  assert.match(cssSource, /\/\* Token mobile action rail:[\s\S]*\.action-label-full[\s\S]*display: none !important;/);
  assert.match(cssSource, /\/\* Token mobile action rail:[\s\S]*\.action-label-mobile[\s\S]*display: inline !important;/);
