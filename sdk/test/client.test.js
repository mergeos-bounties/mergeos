...
test('creates public feed and ledger verification requests without auth', async () => {
  ...
  assert.equal(fetchImpl.calls[1].url, 'https://mergeos.shop/api/public/protocol/tasks?limit=80');
  assert.equal(fetchImpl.calls[1].options.headers.Authorization, undefined);
  assert.equal(fetchImpl.calls[2].url, 'https://mergeos.shop/api/public/protocol/events?limit=80');
  assert.equal(fetchImpl.calls[2].options.headers.Authorization, undefined);
  assert.equal(fetchImpl.calls[3].url, 'https://mergeos.shop/api/public/ledger/verify');
